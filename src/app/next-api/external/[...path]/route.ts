...
// 프록시 대상 URL
const TARGET_URL =
  process.env.NEXT_PUBLIC_LABELSTUDIO_URL || 'http://121.126.210.2/labelstudio';
...
async function handleProxy(request: NextRequest) {
  try {
    ...
    const jwtToken = request.cookies.get('ls_access_token')?.value;
    if (jwtToken) {
      headers['Authorization'] = `Bearer ${jwtToken}`;
      console.log('[PROXY] Added JWT token to Authorization header');
    }
